...
public class RpsGame {
	...
	public void startGame() {
		...
		// 배경 이미지 설정
		ImageIcon imgBack = new ImageIcon("image/배경화면.jpg"); // 배경 이미지 생성
		...
		// VS 이미지
		ImageIcon imgVs = new ImageIcon("image/vs.png"); // VS 이미지 불러옴
		...
		Timer timer = new Timer(100, new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				if (userChoice == -1) { // userchoice의 변수값이 맨처음 지정된 -1일 경우
	...
	private ImageIcon[] createImageArray(String prefix, String[] rpsImg) {
		...
		for (int i = 0; i < 3; i++) {
			String filename = "image/" + prefix + rpsImg[i] + ".png";
	...
	private ImageIcon createImageIcon(String prefix, int choice) {
		return new ImageIcon("image/" + prefix + getRpsImgName(choice) + ".png");
	}
