	...
	public void show_register() {
		...
		ItemListener radioItemListener = new ItemListener() {
			...
			public void itemStateChanged(ItemEvent e) {
				String gender = e.getItem().toString();
				user.setGender(gender);

			}
		};

		male.addItemListener(radioItemListener);
		female.addItemListener(radioItemListener);
		...
		register_b.addActionListener(new ActionListener() // 회원가입 버튼 누르면 데이터 저장
		{
			...
			public void actionPerformed(ActionEvent e) {
				String id = text_id.getText(); // 아이디 입력한 값을 user - id에 저장
				String pw = text_pw.getText(); // 비밀번호 입력한 값을 user - pw에 저장
				String name = text_name.getText(); // 이름 입력한 값을 user - name에 저장
				String tel = text_tel.getText(); // 전화번호 입력한 값을 user - tel에 저장
